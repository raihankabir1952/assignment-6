...
const WorkoutCard = ({ workout }: WorkoutCardProps) => {
  return (
    <Link href={`/workout/${workout.id}`}>
      <article className="group overflow-hidden rounded-xl border border-gray-800 bg-[#17181c] transition-all duration-300 hover:-translate-y-1 hover:border-[#CCFF00]/40 cursor-pointer">
        ...
        <div className="p-5">
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            {workout.muscleGroups.map((group) => (
              <span
                key={group}
                className="rounded-md bg-[#CCFF00]/10 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-[#CCFF00]"
              >
                {group}
              </span>
            ))}
